...
- `GET /api/invest-weather/cs2`
- `GET /api/invest-weather/cs2/block-detail`

说明：

- 以上接口在 Next.js 侧实现（`frontend/app/api/invest-weather/*/route.ts`），不是 FastAPI 路由。
- 纳斯达克 / 标普500 / 黄金模块数据源为 FRED 公共序列，接口做了本地文件缓存（当前 30 分钟刷新一次）。
- CS2 饰品模块数据源为 SteamDT 首页市场看板与板块接口，接口做了本地文件缓存（当前 2 分钟刷新一次）。
- 四个页面都支持状态标签、日内变动、火花线、详情弹窗、历史图表 hover 提示。
- CS2 页面额外支持板块默认排行、上涨排行、下跌排行展示。
- CS2 页面中的“热门板块 / 一级板块”列表项支持点击打开板块详情弹窗。
- 板块详情弹窗当前支持：
  - 今日 / 昨日 / 最高 / 最低 / 涨跌值 / 涨跌幅
  - 板块价格走势图（支持鼠标 hover 交互提示）
  - 成分饰品涨幅榜 / 跌幅榜切换（Top 10）
  - 加载中进度条与骨架占位
- `GET /api/invest-weather/cs2/block-detail` 用于板块详情弹窗，查询参数为 `type`、`typeVal`、`level`。
- 当前板块详情已稳定接入 `relation` 与 `skin-list` 数据；`trend` 接口在部分环境下会触发 SteamDT 风控，因此页面默认只展示可稳定抓取的价格线，不强依赖在售数量线。
- 首页“工具”中的“投资气象站”卡片已接入与“新番导视”同风格的轮播展示（纳指 / 标普 / 黄金 / CS2 行情速览）。
...
│   │   └── invest-weather-station/ # 投资气象站（首页 + nasdaq/sp500/gold/cs2）
│   │   └── console/anime-crawler/ # 新番爬虫控制台
│   └── api/
│       └── invest-weather/    # 投资气象站 Next API（nasdaq/sp500/gold/cs2 + cs2/block-detail）
│   └── settings/
│       ├── account/           # 账号信息修改
│       ├── password/          # 密码修改
│       └── permissions/       # 权限组管理
...
- `/apps/invest-weather-station` 投资气象站首页（纳指/标普/黄金/CS2 入口）
...
- `/apps/invest-weather-station/cs2` CS2 饰品交易气象站
- `GET /api/invest-weather/nasdaq|sp500|gold` 投资气象站 Next API（30 分钟缓存刷新）
- `GET /api/invest-weather/cs2` 投资气象站 CS2 Next API（2 分钟缓存刷新）
- `GET /api/invest-weather/cs2/block-detail?type=...&typeVal=...&level=...` CS2 板块详情 API（板块价格走势 + 成分榜）
