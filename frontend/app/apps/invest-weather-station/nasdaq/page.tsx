...
type NasdaqWeatherResponse = {
  generatedAt: string;
  lastUpdatedAt?: string;
  cache?: {
    refreshIntervalMinutes?: number;
  };
  sections: NasdaqSection[];
};
...
export default function NasdaqWeatherStationPage() {
  const [data, setData] = useState<NasdaqWeatherResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const response = await fetch("/api/invest-weather/nasdaq", {
          method: "GET"
        });
        const json = (await response.json()) as NasdaqWeatherResponse | { error?: string };
        if (!response.ok) {
          throw new Error("暂时无法加载纳斯达克气象站数据");
        }
        if (active) {
          setData(json as NasdaqWeatherResponse);
        }
      } catch (err) {
        if (active) {
      ...
      <AppShell>
        <div className="space-y-8">
          <div className="mb-8 flex items-center justify-between gap-3">
            <div>
              <h1 className="text-2xl font-semibold text-slate-900">纳斯达克宏观气象站</h1>
