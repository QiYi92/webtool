...
export default function Sp500WeatherStationPage() {
  const [data, setData] = useState<Sp500WeatherResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const response = await fetch("/api/invest-weather/sp500", {
          method: "GET"
        });
        const json = (await response.json()) as Sp500WeatherResponse | { error?: string };
        if (!response.ok) {
          throw new Error("暂时无法加载标普500气象站数据");
        }
        if (active) {
          setData(json as Sp500WeatherResponse);
        }
      } catch (err) {
        if (active) {
      ...
      <AppShell>
        <div className="space-y-8">
          <div className="mb-8 flex items-center justify-between gap-3">
            <div>
              <h1 className="text-2xl font-semibold text-slate-900">标普500宏观气象站</h1>
