...
export async function GET() {
  ...
  if (isUsableCache(cached)) {
    void triggerRefresh().catch((error) => {
      console.error("[invest-weather:gold] background refresh failed", error);
    });
  ...
  try {
    ...
    return jsonNoStore({ error: "Failed to fetch FRED data for gold station" }, { status: 500 });
  } catch (error) {
    console.error("[invest-weather:gold] refresh failed without cache", error);
    return jsonNoStore({ error: "Failed to fetch FRED data for gold station" }, { status: 500 });
  }
}
