...
export async function GET() {
  ...
  if (isUsableCache(cached)) {
    void triggerRefresh().catch((error) => {
      console.error("[invest-weather:sp500] background refresh failed", error);
    });
  ...
  try {
    ...
    return jsonNoStore({ error: "Failed to fetch FRED data for sp500 station" }, { status: 500 });
  } catch (error) {
    console.error("[invest-weather:sp500] refresh failed without cache", error);
    return jsonNoStore({ error: "Failed to fetch FRED data for sp500 station" }, { status: 500 });
  }
}
