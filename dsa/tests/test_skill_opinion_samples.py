...
def test_sample_repository_compiles_postgresql_upsert() -> None:
    class CapturingSession:
        ...
        def execute(self, statement):
            self.calls += 1
            if self.calls == 1:
                return SimpleNamespace(scalars=lambda: iter([1]))
    ...
    class PostgreSQLDatabase:
        ...
        def _run_write_transaction(self, _operation_name, operation):
            return operation(self.session)

    db = PostgreSQLDatabase()
    created = SkillOpinionSampleRepository(db_manager=db).insert_missing(
        [
            {
    ...
    assert created == 1
    compiled = str(db.session.statement.compile(dialect=postgresql.dialect()))
