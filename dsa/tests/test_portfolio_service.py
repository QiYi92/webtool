...
class PortfolioServiceTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # The embedded service may set DSA_DATABASE_URL to Supabase.  These
        # tests must always use their own disposable SQLite database instead.
        self._previous_dsa_database_url = os.environ.pop("DSA_DATABASE_URL", None)
    ...
    def tearDown(self) -> None:
        ...
        if self._previous_dsa_database_url is not None:
            os.environ["DSA_DATABASE_URL"] = self._previous_dsa_database_url
        else:
            os.environ.pop("DSA_DATABASE_URL", None)
    ...
    def test_portfolio_write_session_does_not_issue_sqlite_lock_statement_on_postgresql(self) -> None:
        repo = PortfolioRepository(db_manager=self.db)
        session = self.db.get_session()
        connection = session.connection()

        with patch.object(self.db, "get_session", return_value=session):
            with patch.object(self.db, "_is_sqlite_engine", False):
                with patch.object(connection, "exec_driver_sql") as mock_exec:
                    with repo.portfolio_write_session():
                        pass
