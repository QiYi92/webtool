...
class SkillOpinionSampleRepository:
    ...
    def insert_missing(self, rows: Iterable[Dict[str, Any]]) -> int:
        ...
        def _insert(session):
            ...
            # The two dialects expose the same SQLAlchemy conflict API, but a
            # SQLite Insert cannot be compiled by PostgreSQL.  Select the
            # concrete statement factory from the active database engine.
