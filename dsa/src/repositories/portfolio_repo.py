...
class PortfolioRepository:
    ...
    def portfolio_write_session(self):
        ...
        try:
            # ``BEGIN IMMEDIATE`` obtains SQLite's writer lock, but it is not
            # valid PostgreSQL syntax.  SQLAlchemy opens a normal transaction
            # for PostgreSQL automatically on the first statement, which is
            # the appropriate transaction model for Supabase.
