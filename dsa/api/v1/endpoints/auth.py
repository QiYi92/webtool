...
def _cookie_params(request: Request) -> dict:
    ...
    force_secure = os.getenv("DSA_COOKIE_SECURE", "").lower()
    if force_secure in {"true", "1", "yes"}:
        secure = True
    elif force_secure in {"false", "0", "no"}:
        secure = False
    elif os.getenv("TRUST_X_FORWARDED_FOR", "false").lower() == "true":
...
async def auth_update_settings(request: Request, body: AuthSettingsRequest):
    ...
    if os.getenv("DSA_SSO_ENABLED", "false").lower() in ("true", "1", "yes"):
        return JSONResponse(
            status_code=403,
            content={"error": "sso_managed", "message": "DSA authentication is managed by the tool station"},
        )
...
async def auth_login(request: Request, body: LoginRequest):
    ...
    if os.getenv("DSA_SSO_ENABLED", "false").lower() in ("true", "1", "yes"):
        return JSONResponse(
            status_code=403,
            content={"error": "sso_managed", "message": "Please open DSA from the tool station"},
        )
