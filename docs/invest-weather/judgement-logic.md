# 投资气象站判定逻辑与数据口径（纳指/标普/黄金/港股/A股）

更新时间（北京时间）：2026-07-22
数据来源：FRED、腾讯公开 K 线、东方财富公开行情接口；历史阈值参考 `nasdake.top`

## 1. 说明

本文档沉淀五个投资气象站模块的指标判定、数据源和图表口径。纳指、标普、黄金的阈值最初参考 `nasdake.top`，港股与 A 股为本项目扩展。
...
上述 A 股指数按最近两个交易日收盘涨跌判定状态：上涨为 `success`，小幅回落为 `warning`，跌幅不低于 1% 为 `danger`，无有效变化为 `neutral`。
...
- 五个子页统一调用 `MarketWeatherDashboard`，专业图表统一由 `ProfessionalMarketChart`（KLineChart）渲染。
- A 股宽基、恒生指数、恒生科技、红利低波、纳斯达克综合指数和标普500使用真实 OHLC 蜡烛图。
- 纳指/标普 OHLC 主源为腾讯公开指数 K 线（`.IXIC` / `.INX`），东方财富为备用源；两者失败时保留 FRED 单值折线。
- 宏观、利率、比值和资金流等单值序列只使用专业折线模式，不由单值伪造 OHLC。
- 当日展示数据源提供的真实分时数据；无分时源时显示空状态。
- 日 K、周 K、月 K各截取最近180个周期点。周 K采用周内首个开盘、最高价、最低价、最后收盘；月 K同理按月聚合。历史不足180个周期时不补造数据。
- 接口保留完整 OHLC 日线用于周/月聚合，卡片统计与火花线仍使用最近180个单值历史点。
